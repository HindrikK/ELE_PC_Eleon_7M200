...
//
//*******************************************************************
#ifndef FILE_IO_T1_V5_H
#define FILE_IO_T1_V5_H
...
STATUS InitIoDriver(OS_MEMORY_POOL *MemoryPool,
                     const U16 Entries,
                     UNSIGNED *MyQueue,
                     SUBSCRIPTION *LocalSubscription,
                     const U16 NumberOfIoDrivers,
                     UNSIGNED *Message,
                     U8 MessageSize,
                     struct IoDriverStruct *IoDriverTable,
                     U8 AppState);

#define VARIABLE_TYPE_CHECK(VarID, Type, Value) STATUS_LINE_VAR_RETURN_VALUE(OS_VarDB_GetVariableType(VarID, &Type)); if(sizeof(Value) != (Type & TYPE_LENGTH_MASK)) {STATUS_LINE_VAR(-1)};

// ---------------------------------------------------------------------
// This module uses these variables and functions from other modules.
// ---------------------------------------------------------------------
char *AppBackupPosition(void);
extern const unsigned int IoDriverLocalVarsSize;
extern struct os_db_local *IoDriverLocalVarsPtr;

extern OS_TASK IoDriverOsVar;
extern OS_QUEUE LocalQueue;
